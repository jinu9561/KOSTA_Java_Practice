...
public class BankService {
	...
	int ACCOUNT_CURRENT_SIZE; /** 현재계좌의 개수 */ //count역할
	int USER_CURRENT_SIZE; /**현재 고객의 개수 */ //count역할
	...
	*/
	public AccountDto[] getAccountList(int userSeq) {
		AccountDto[] tempAccountList = new AccountDto[ACCOUNT_CURRENT_SIZE++];
		int resAccountIdx = 0;
		
		for(int i = 0; i < ACCOUNT_CURRENT_SIZE-1; i++) 
			if(accountList[i].userSeq == userSeq) tempAccountList[resAccountIdx++] = accountList[i];
		
		AccountDto[] resAccountList = new AccountDto[resAccountIdx];
		
		if(resAccountIdx != 0) {
			for(int i = 0; i < resAccountIdx; i++) resAccountList[i] = tempAccountList[i];
	...
	**/
	public UserDto getUserDetail(int userSeq) {
		for(int i = 0; i < USER_CURRENT_SIZE-1; i++) 
			if(userList[i].userSeq == userSeq) return userList[i];
		return null;
	}
